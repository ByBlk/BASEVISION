            ...
            Set = function(member, ped)
                ...
                if member.pedheadshot ~= -1 then
                    ...
                    if not IsTextureDictLoaded then
                        console.debug("ERROR: BikerClubhouseDrawMembers - Textures dictionnary \"" .. tostring(member.textureDict) .. "\" cannot be loaded.")
                    end
                else
                    console.debug("ERROR: BikerClubhouseDrawMembers - PedHeadShot not ready.")
                end
            end,
        ...
        MissionsWall = {
            ...
            Init = function()
                if not DrawEmptyRect(BikerGang.Clubhouse.MissionsWall.target, BikerGang.Clubhouse.MissionsWall.prop) then
                    console.debug("ERROR: BikerGang.Clubhouse.MissionsWall.Init() - DrawEmptyRect - Timeout")
                end
            end,
            Enable = function(state)
...
function DrawEmblem(texturesDict, rotation)
    if BikerGang.Clubhouse.Emblem.stage == 0 then
        ...
        if not IsTextureDictLoaded then
            console.debug("ERROR: DrawEmblem - Textures dictionnary cannot be loaded.")
        end
