...
function GetPedheadshotTexture(ped)
    ...
    if pedheadshot ~= -1 then
        ...
        if not IsTextureDictLoaded then
            console.debug("ERROR: GetPedheadshotTexture - Textures dictionnary \"" .. tostring(textureDict) .. "\" cannot be loaded.")
        end
    else
        console.debug("ERROR: GetPedheadshotTexture - PedHeadShot not ready.")
    end
