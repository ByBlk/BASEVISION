...
function VFW.OneSync.GetPlayersInArea(source, maxDistance, ignore)
...
---@param maxDistance number
---@param ignore? table playerIds to ignore, where the key is playerId and value is true
function VFW.OneSync.GetClosestPlayer(source, maxDistance, ignore)
...
---@return number? netId
function VFW.OneSync.SpawnVehicle(model, coords, heading, properties, cb)
    ...
    CreateThread(function()
        local xPlayer = VFW.OneSync.GetClosestPlayer(coords, 300)
        ...
        VFW.GetVehicleType(vehicleModel, xPlayer.id, function(vehicleType)
            print(vehicleType)
            ...
            while not createdVehicle or createdVehicle == 0 or NetworkGetEntityOwner(createdVehicle) == -1 do
                ...
                if tries > 40 then
                    if promise then
                        return promise:reject(("Could not spawn vehicle - ^3%s^7!"):format(model))
                    end
                    error(("Could not spawn vehicle - ^3%s^7!"):format(model))
                end
            end
...
---@param heading number
---@param cb function
function VFW.OneSync.SpawnObject(model, coords, heading, cb)
...
---@param heading number
---@param cb function
function VFW.OneSync.SpawnPed(model, coords, heading, cb)
    if type(model) == "string" then
        model = joaat(model)
    end
    CreateThread(function()
...
---@param seat number
---@param cb function
function VFW.OneSync.SpawnPedInVehicle(model, vehicle, seat, cb)
    if type(model) == "string" then
        model = joaat(model)
    end
    CreateThread(function()
...
---@return table
function VFW.OneSync.GetPedsInArea(coords, maxDistance, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return table
function VFW.OneSync.GetObjectsInArea(coords, maxDistance, modelFilter)
...
---@return table
function VFW.OneSync.GetVehiclesInArea(coords, maxDistance, modelFilter)
...
function VFW.OneSync.GetClosestPed(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function VFW.OneSync.GetClosestObject(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function VFW.OneSync.GetClosestVehicle(coords, modelFilter)
