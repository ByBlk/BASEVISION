...
LaboTable = {
    ...
    PedConfig = function(ped,scene,animDict,animName,blendIn,blendOut,duration,flag,speed,unk1)
        ...
        if not ped      then console.debug("[Core] LaboTable.PedConfig NO PED.");         return false; end
        if not scene    then console.debug("[Core] LaboTable.PedConfig NO SCENE.");      return false; end
        if not animDict then console.debug("[Core] LaboTable.PedConfig NO animDict.");   return false; end
        if not animName then console.debug("[Core] LaboTable.PedConfig NO animName.");  return false; end
    ...
    EntityConfig = function(entity,scene,animDict,animName,blendIn,blendOut,flags)
        if not entity   then console.debug("[Core] LaboTable.EntityConfig NO ENTITY.");      return false; end
        if not scene    then console.debug("[Core] LaboTable.EntityConfig NO SCENE.");      return false; end
        if not animDict then console.debug("[Core] LaboTable.EntityConfig NO ANIMDICT.");   return false; end
        if not animName then console.debug("[Core] LaboTable.EntityConfig NO ANIMNAME.");  return false; end
...
function LocalLaboPed(Q, labid, iPed) -- ## quand on sort labo, ça delete le ped
    CreateThread(function()
        ...
        SetPedAsNoLongerNeeded(iPed)
        SetEntityAsMissionEntity(iPed, false, false)
    end)
end

local function LocalCreatePed(Q, labid)
    local hash = GetHashKey(Q.Ped)
    RequestModel(hash)
    while not HasModelLoaded(hash) do Wait(1) end
    local iPed = CreatePed(1, hash, Q.location, 255, 1)
...
function CreateLaboPeds(typeD, labid)
    ...
    if DRUG_DATA[typeD] then
        for k, v in pairs(DRUG_DATA[typeD]) do
            LocalCreatePed(v, labid)
        end
    else
        console.debug("[^1ERROR Labo^7] Table DRUG_DATA[typeD] "..typeD.." n'existe pas, MERCI DE LE REPORT")
    end
end