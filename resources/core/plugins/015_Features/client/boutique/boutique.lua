...
function GetVehiculesBoutique()
    -- Faut faire truc pour trouver la cat�gorie "VehiculesBoutique"
    -- au final flemme 
    return "t�te de noeil fr�re"
end
...
OpenMenuVehBoutique = function(veh, data)
    ...
    if data.marque == "H�licopt�re" then 
        data.category = "heli"
        coordsVehBoutique = vector4(-1266.8576660156, -3013.1684570313, -49.490184783936, 40.814487457275)
        camCoords = vector3(-1271.4304199219, -3002.7814941406, -46.489810943604)
        PinInteriorInMemory(GetInteriorAtCoords(-1266.8576660156, -3013.1684570313, -49.490184783936))        
    ...
    if not VFW.Game.IsSpawnPointClear(coordsVehBoutique.xyz, 3.0) then
        for k,v in pairs(VFW.Game.GetVehiclesInArea(coordsVehBoutique.xyz, 3.0)) do 
            DeleteEntity(v)
        end
    end
    ...
    if GetGameTimer() - cooldownVeh > 1000 then
        ...
        while GetGameTimer() - cooldownVeh < 1000 do 
            Wait(1)
            LoadingPrompt("Chargement du v�hicule...", 2)
        end
        BusyspinnerOff()
        cooldownVeh = GetGameTimer()
    end
    ...
    VehBoutique = cEntity.Manager:CreateVehicleLocal(veh, coordsVehBoutique.xyz, coordsVehBoutique.w)
...
RegisterNUICallback('nui:boutique-razv4:close', function(data, cb)
...
RegisterNetEvent("core:boutiqueveh:try", function(name, category, oldcoords, colors, perf)
    ...
    SetEntityCoords(PlayerPedId(), gcoords.xyz)
    local vehicle2 = cEntity.Manager:CreateVehicleLocal(name, gcoords.xyz, gcoords.w)
    local plate = VFW.Game.GetVehicleProperties(vehicle2.id).plate
    ...
    exports['VisionHUD']:toggleVehicleInTry(true, model)
    ...
    while inTryVeh do
        ...
        if GetGameTimer() > timer then
            ...
            -- ShowNotification("~r~Le d�lai a �t� d�pass�")
            exports['VisionHUD']:toggleVehicleInTry(false, nil)
            -- New notif
            exports['vNotif']:createNotification({
                type = 'ROUGE',
                -- duration = 5, -- In seconds, default:  4
                content = "~s Le d�lai a �t� d�pass�"
            })
        ...
        elseif IsControlJustPressed(0, 178) then
            -- ShowNotification("~r~Vous avez annul� le test")
            exports['VisionHUD']:toggleVehicleInTry(false, nil)
            -- New notif
            exports['vNotif']:createNotification({
                type = 'JAUNE',
                -- duration = 5, -- In seconds, default:  4
                content = "Vous avez ~s annul� ~c le test"
            })
            TriggerServerEvent("core:boutique:finishedtest")
...
AddEventHandler("core:spawnboutiqueVehiclePayant", function(model, props, category)
    ...
    for k,v in pairs(VehBoutiqueSpawns) do 
        if VFW.Game.IsSpawnPointClear(v.xyz, 3.0) then
            goodCoords = v
            break
        end
    end
    ...
    local vehEntity = cEntity.Manager:CreateVehicle(model, vec, vec.w)
    local veh = vehEntity.id
    VFW.Game.SetVehicleProperties(veh, props)
    TriggerServerEvent("core:SetVehicleOut", string.upper(props.plate), VehToNet(veh), veh)
    TriggerServerEvent("core:SetPropsVeh", token, all_trim(props.plate), VFW.Game.GetVehicleProperties(veh))
    ...
    if props and props.modBrakes then 
        ...
        TriggerServerEvent("core:SetPropsVeh", token, all_trim(GetVehicleNumberPlateText(veh)), VFW.Game.GetVehicleProperties(veh))
    end
    if timere < 600 then
...
function SpawnVehBoutique(vehent, category)
    --[[ exports['vNotif']:createNotification({
        ...
        mainMessage = "Vous venez d'acheter ce v�hicule !",
        duration = 10,
    }) ]]
