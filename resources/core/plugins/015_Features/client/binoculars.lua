...
local function UseBinocular()
    ...
    if binoculars then
        CreateThread(function()
            TaskStartScenarioInPlace(VFW.PlayerData.ped, "WORLD_HUMAN_BINOCULARS", 0, 1)
            PlayAmbientSpeech1(VFW.PlayerData.ped, "GENERIC_CURSE_MED", "SPEECH_PARAMS_FORCE")
        end)
        ...
        while binoculars and not IsEntityDead(VFW.PlayerData.ped) and not IsPedSittingInAnyVehicle(VFW.PlayerData.ped) do
            VFW.ShowHelpNotification("Appuyer sur ~INPUT_CELLPHONE_CANCEL~ pour arrêter de regarder")
