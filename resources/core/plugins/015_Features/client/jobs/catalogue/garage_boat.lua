...
local function loadGarage()
    while not VFW.Jobs or not VFW.Jobs.Catalogue or not VFW.Jobs.Catalogue.GarageBoat do
        Wait(100)
    end

    while not next(VFW.Jobs.Catalogue.GarageBoat) do
        Wait(100)
    end

    if not lastJob or not VFW.Jobs.Catalogue.GarageBoat[lastJob] then return end

    local garageData = {}
    local lastEntity = false
    local lastModel = false
    local config = VFW.Jobs.Catalogue.GarageBoat[lastJob]
    ...
    local function getGarageData(category, firstLabel, secondLabel)
        local data = {
            ...
            eventName = ("garage_boat_%s"):format(lastJob),
            category = { show = false },
            cameras = { show = false },
    ...
    function VFW.Jobs.Catalogue.GarageBoat.MenuGarage()
        local cam = config.Cam
        VFW.Cam:Create(("garage_boat_%s"):format(lastJob), cam)
        VFW.Nui.NewGrandMenu(true, getGarageData(config.Nui.lastCategory))
        cEntity.Visual.HideAllEntities(true)
    end

    RegisterNuiCallback(("nui:newgrandcatalogue:garage_boat_%s:selectHeadCategory"):format(lastJob), function(data)
    ...
    RegisterNuiCallback(("nui:newgrandcatalogue:garage_boat_%s:selectGridType"):format(lastJob), function(data)
        local model = data
        ...
        if model then
            ...
            if lastModel ~= model then
                ...
                local cam = config.Cam.COH

                lastEntity = CreateVehicle(model, cam.x, cam.y, cam.z, cam.w, false, false)
    ...
    RegisterNuiCallback(("nui:newgrandcatalogue:garage_boat_%s:selectBuy"):format(lastJob), function(data)
        ...
        for key in pairs(config.Vehicle.Spawn) do table.insert(spawnPoint, tonumber(key)) end
        table.sort(spawnPoint)

        deleteCar()

        VFW.Cam:Destroy(("garage_boat_%s"):format(lastJob))
        ...
        for key, _ in ipairs(spawnPoint) do
            ...
            local value = config.Vehicle.Spawn[tostring(key)]
    ...
    RegisterNuiCallback(("nui:newgrandcatalogue:garage_boat_%s:close"):format(lastJob), function()
        config.Nui.lastCategory = config.Nui.defaultCategory
        VFW.Nui.NewGrandMenu(false)
        cEntity.Visual.HideAllEntities(false)
        deleteCar()
    ...
    console.debug(("^3[%s]: ^7Garages Boat ^3loaded"):format(VFW.PlayerData.job.label))
end
...
local function deletingZone()
    ...
    VFW.RemoveInteraction(("garage_boat_%s"):format(lastJob))
    VFW.RemoveInteraction(("garageDespawn_boat_%s"):format(lastJob))
