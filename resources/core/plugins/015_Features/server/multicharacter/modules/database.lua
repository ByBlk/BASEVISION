    ...
    if DB_COLUMNS then
        ...
        if next(columns) then
            ...
            if MySQL.transaction.await(queries) then
                console.info(("Updated ^5%s^7 columns to use ^5VARCHAR(%s)^7"):format(count, length))
            else
                console.info(("Unable to update ^5%s^7 columns to use ^5VARCHAR(%s)^7"):format(count, length))
            end
        end

        Database.connected = true

        VFW.Jobs = VFW.GetJobs()
        while not next(VFW.Jobs) do
            Wait(500)
            VFW.Jobs = VFW.GetJobs()
        end

        VFW.Factions = VFW.GetFactions()
...
function Database:DeleteCharacter(source, charid)
    ...
    MySQL.transaction(queries, function(result)
        if result then
            ...
            console.info(("Player ^5%s %s^7 has deleted a character ^5(%s)^7"):format(name, source, identifier))
            Wait(50)
...
function Database:GetPlayerInfo(identifier, slots)
    return MySQL.query.await(
        "SELECT id, identifier, accounts, job, job_grade, firstname, lastname, dateofbirth, sex, skin, disabled, mugshot, tattoos FROM users WHERE identifier LIKE ? LIMIT ?",
        { identifier, slots })
end
